...
namespace = Namespace("tylermarquescom", metadata=ObjectMetaArgs(name="tylermarquescom"))
...
deployment = k8s.apps.v1.Deployment(
    ...
    metadata=ObjectMetaArgs(name="tylermarques-com", namespace=namespace.metadata.name, labels={"app": "tylermarquescom"}),
    spec=DeploymentSpecArgs(
        selector=LabelSelectorArgs(match_labels={"app": "tylermarquescom"}),
        replicas=1,
        strategy=DeploymentStrategyArgs(type="RollingUpdate", rolling_update=RollingUpdateDeploymentArgs(max_surge="25%", max_unavailable="25%")),
        template=k8s.core.v1.PodTemplateSpecArgs(
            metadata=ObjectMetaArgs(
                labels={"app": "tylermarquescom"},
                annotations={"kubectl.kubernetes.io/default-container": "tylermarquescom"},
            ),
            spec=k8s.core.v1.PodSpecArgs(
                containers=[
                    k8s.core.v1.ContainerArgs(
                        ...
                        ports=[k8s.core.v1.ContainerPortArgs(container_port=3000, name="http", protocol="TCP")],
                    )
                ],
                image_pull_secrets=[k8s.core.v1.LocalObjectReferenceArgs(name="gchrio-cred")],
            ),
        ),
    ),
)
...
marquescg_com = k8s.yaml.ConfigFile("tylermarquescom", os.path.join(current_dir, "argoApp.yaml"))
