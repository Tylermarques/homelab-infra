...
def create_sabnzbd_deployment(
    namespace=media_namespace.metadata.name,
    enabled=True,
    ingress_host="sabnzbd.local",
    puid=1000,
    pgid=1000,
    nfs_config={
                ...
                spec=k8s.core.v1.PodSpecArgs(
                    ...
                    containers=[
                        k8s.core.v1.ContainerArgs(
                            ...
                            image=f"{image}:{tag}",
                            ports=[
                                k8s.core.v1.ContainerPortArgs(container_port=http_port, name="http"),
                                k8s.core.v1.ContainerPortArgs(container_port=https_port, name="https"),
                            ],
                            env=env_vars,
    ...
    service = k8s.core.v1.Service(
        ...
        spec=k8s.core.v1.ServiceSpecArgs(
            type=service_type,
            ports=[
                k8s.core.v1.ServicePortArgs(port=http_port, target_port="http", name="http"),
                k8s.core.v1.ServicePortArgs(port=https_port, target_port="https", name="https"),
            ],
            selector=app_labels,
        ),
    )

    # Sabnzbd is local-only, no ingress needed
    ingress = None
...
sabnzbd = create_sabnzbd_deployment()
if sabnzbd:
    pulumi.export("sabnzbd-pv", sabnzbd["pv"].metadata.name)
    pulumi.export("sabnzbd-pvc", sabnzbd["pvc"].metadata.name)
    pulumi.export("sabnzbd-deployment", sabnzbd["deployment"].metadata.name)
    pulumi.export("sabnzbd-service", sabnzbd["service"].metadata.name)
